/**
 * findIndex方法 查找第一个符合条件的元素的下标
 * 参数2个
 * 参数一：回调函数 函数有三个参数 1. item 当前遍历元素 2. 当前遍历下标 3. 遍历的目标数组 符合条件返回true否则返回false
 * 参数二：回调函数的this
 * 返回值：返回第一个查找到的元素的下标，没找到返回-1
 */
const arr1 = [1, 2, 3, 4, 5]
console.log(
  arr1.findIndex((item) => item > 10),
  arr1.findIndex((item) => item > 3)
)
...
function myFind(callback, thisObj) {
  ...
  for (let i = 0; i < context.length; i++) {
    if (callback?.apply(thisObj, [context[i], i, context])) return i
  }
  return -1
}
