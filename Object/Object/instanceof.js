/**
 * instanceof 关键字
 * instanceof 关键字主要用于判断 实例 是否是目标 类的实例
 */
...
function myInstanceOf(instance, classConstructor) {
  if (typeof classConstructor !== 'function') {
    throw new Error('classConstructor must be a Function')
  }
  let instancePrototype = Object.getPrototypeOf(instance)
  const classConstructorPrototype = classConstructor.prototype
  while (true) {
    if (instancePrototype === null) return false
    if (classConstructorPrototype === instancePrototype) return true
